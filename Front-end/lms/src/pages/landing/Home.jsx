...
import { faBookOpen, faClock, faChevronLeft, faChevronRight, faExternalLinkAlt, faStarHalfAlt, faCode, faLaptopCode, faServer, faLayerGroup, faPuzzlePiece, faBullseye, faBullhorn } from "@fortawesome/free-solid-svg-icons";
...
function Home() {
    ...
    // Auto-rotate banners (pause when tab not visible)
    useEffect(() => {
    ...
    useEffect(() => {
        if (!testimonials.length || testimonialPaused) return;
        const t = setInterval(() => {
            ...
            try {
                ...
                if (res.success && Array.isArray(res.data) && res.data.length) {
                    const mapped = res.data.map((c, idx) => ({
                        ...
                        profileImage: c.user?.profileImage, // Base64 image
                        user: c.user, // Keep full user object for avatar handling
    ...
    const CourseThumbnail = ({ course, getCourseImage, getThumb }) => {
        const [imageError, setImageError] = useState(false);
        const imageUrl = getCourseImage(course);
        const fallback = getThumb(course?.course_name || "");
        const shouldShowImage = imageUrl && !imageError && imageUrl !== fallback;

        return (
            <div className="w-24 h-24 rounded-lg flex-shrink-0 bg-slate-100 overflow-hidden relative">
                {shouldShowImage ? (
                    <img
                        src={imageUrl}
                        alt={course.course_name}
        ...
        <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-100/40 via-white to-slate-50 relative">
                                        ...
                                        <div className="flex items-start gap-4">
                                            <div className="relative flex-shrink-0">
                                                <div className="h-16 w-16 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center shadow-sm border-2 border-blue-100 ring-2 ring-blue-50 overflow-hidden relative">
                                                    ...
                                                    {testimonials[testimonialIndex]?.profileImage ? (
                                                        <img
                                                            src={`data:image/jpeg;base64,${testimonials[testimonialIndex].profileImage}`}
            ...
            {showScrollTop && (
                <button
                    ...
                    className="fixed bottom-6 right-6 w-11 h-11 rounded-full bg-indigo-600 text-white shadow-sm hover:shadow-md hover:bg-indigo-700 transition-all duration-200 z-50 flex items-center justify-center"
